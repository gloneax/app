// src/components/HepatitisMap.tsx
import React, { useEffect, useState } from 'react';
import { MapContainer, TileLayer, CircleMarker, Popup } from 'react-leaflet';
import 'leaflet/dist/leaflet.css';

interface HepatitisRecord {
  countryCode: string;
  countryName: string;
  year: number;
  hepatitisCases: number;
  displayCases: string;
  coordinates: [number, number];
}

export default function HepatitisMap() {
  const [data, setData] = useState<HepatitisRecord[] | null>(null);
  const [loading, setLoading] = useState<boolean>(true);

  useEffect(() => {
    const fetchHepatitisData = async () => {
      try {
        const response = await fetch('/api/hepatitis');
        const json = await response.json();
        if (Array.isArray(json)) {
          setData(json);
        }
      } catch (error) {
        console.error("Error fetching Hepatitis mapping metrics:", error);
      } finally {
        setLoading(false);
      }
    };
  ...
  // 🌟 Color codes strictly mapped to match your legend's color palette
  const getMarkerProperties = (prevalence: number) => {
    if (prevalence >= 5.0) {
      return { color: "#4c0519", fillColor: "#881337", radius: 18 }; // High Endemic (Burgundy)
    }
    if (prevalence >= 2.0) {
      return { color: "#7c2d12", fillColor: "#c2410c", radius: 13 }; // Intermediate-High (Orange)
    }
    if (prevalence >= 1.0) {
      return { color: "#713f12", fillColor: "#b45309", radius: 9 };  // Intermediate-Low (Amber)
    }
    ...
    return (
      <div className="h-full w-full flex items-center justify-center bg-slate-50 dark:bg-slate-950 font-sans text-slate-500 font-medium">
        <div className="flex flex-col items-center gap-2">
          ...
          <span>Ingesting WHO Global Chronic Hepatitis B Prevalence...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full w-full relative z-0 [&_.leaflet-container]:!bg-[#abd3df]">
      <MapContainer center={[12, 5]} zoom={2.5} className="h-full w-full absolute inset-0">
        ...
        {data && data.map((record) => {
          ...
          const displayVal = record.displayCases || `${record.hepatitisCases.toFixed(1)}%`;

          const styles = getMarkerProperties(record.hepatitisCases || 0);
